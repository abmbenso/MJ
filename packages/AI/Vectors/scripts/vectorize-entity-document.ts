// vectorize-entity-document.ts -- test-vectorize-entity.ts restricted to ONE entity document by name.
// Usage (repo root): npx tsx packages/AI/Vectors/scripts/vectorize-entity-document.ts "IBTR Appeals Search"
...
async function main(): Promise<void> {
    ...
    const entityDocumentType = "Search"; const onlyName = process.argv[2]; if (!onlyName) throw new Error("usage: vectorize-entity-document.ts \"<Entity Document Name>\"");
    ...
    const entityDocuments: MJEntityDocumentEntity[] = (await vectorizer.GetActiveEntityDocuments(undefined, entityDocumentType)).filter(d => d.Name === onlyName); if (!entityDocuments.length) throw new Error(`no active Search entity document named ${onlyName}`);
