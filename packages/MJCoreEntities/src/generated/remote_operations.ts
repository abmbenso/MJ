...
/** Input for `ClientPortfolio.Import` (spec §3.4). Rows are already parsed by the caller; the file bytes travel for provenance only. */
export interface ClientPortfolioImportInput {
    /** Existing `Clients.ID`; the operation never creates a client. */
    clientID: string;
    /** Original file name, e.g. "Zidan portfolio 2026.xlsx". */
    fileName: string;
    /** The file's bytes, base64. Max 5 MB decoded. */
    fileBase64: string;
    /** Sheet rows after the caller's column mapping. One per sheet row, in sheet order. */
    rows: ClientPortfolioImportRow[];
    /** Confirmed candidate properties (spec §3.3). Empty = create one Suggested single-parcel property per matched row. */
    properties: ClientPortfolioImportProperty[];
    /** `County Assessment Cycles.ID` values to create appeals for; only cycles in a parcel's county apply to that parcel. */
    cycleIDs: string[];
    /** Match decisions per row, keyed by rowIndex: the parcel the practitioner accepted, or null for not-found. */
    matches: ClientPortfolioImportMatch[];
}
export interface ClientPortfolioImportRow {
    rowIndex: number;
    ...
    /** Every unrecognized column, verbatim. */
    extra: Record<string, string>;
}
export interface ClientPortfolioImportMatch {
    rowIndex: number;
...
export interface ClientPortfolioImportProperty {
    ...
    name: string;
    propertyType: 'Apartment' | 'Office' | 'Retail' | 'Industrial' | 'Hotel' | 'Mixed Use' | 'Land' | 'Other';
    /** rowIndex values of the rows in this property. */
    rowIndexes: number[];
    /** Confirm now (Confirmed) or leave Suggested. */
    confirmed: boolean;
    /** The client's display name for the link, or null. */
    displayName: string | null;
}

/** Output for `ClientPortfolio.Import`. */
export interface ClientPortfolioImportOutput {
    clientImportID: string;
    sourceDocumentID: string;
    /** True when a ClientImport for (client, sheet hash) already existed; nothing was created. */
...
// ============================================================
// ClientPortfolio.Import — Import Client Portfolio
// ============================================================
/**
 * Import Client Portfolio
 * Turns a client's parcel spreadsheet (rows already matched and grouped by the practitioner) into Properties, Property Parcels, Client Properties, one Client Appeal per parcel per open County Assessment Cycle, Client Tasks for not-found rows, and a Client Import audit row. Idempotent on (client, sheet sha256).
 * GenerationType=Manual — the server body is supplied by a hand-authored subclass registered
 * under 'ClientPortfolio.Import'. This generated base provides the typed contract only (client-safe).
 */
