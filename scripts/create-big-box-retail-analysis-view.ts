/**
 * Create a shared default grid view for the "Store Analysis" entity
 * (big_box_retail.vwStoreAnalysis), so the Big Box Retail Application's second nav item
 * (Task 5) has something to point at. Also grants the base "UI" role View access on that
 * view via an `MJ: Resource Permissions` row — `IsShared = true` alone does NOT grant
 * non-owner users access; MJUserViewEntityExtended.CalculateUserCanView()
 * (packages/MJCoreEntities/src/custom/MJUserViewEntityExtended.ts) requires an actual
 * approved Resource Permission row for any non-owner user.
 *
 ...
 * if present.
 *
 * Modeled on scripts/create-big-box-retail-default-view.ts (the "Stores" roster entity's
 * equivalent setup).
 *
 * Usage (repo root): npx tsx scripts/create-big-box-retail-analysis-view.ts
 */
...
// Task 3 confirmed CodeGen assigned the entity name "Store Analysis" for the read-only
// virtual entity over big_box_retail.vwStoreAnalysis (47 fields, 389 rows) — verified via
...
const VIEW_NAME = 'All Big Box Retail Stores — Analysis';

// The "UI" / "Basic UI" role — controller-confirmed standard base role every logged-in
// Explorer user holds, queried directly from __mj.Role. Same role ID used by
// create-big-box-retail-default-view.ts.
...
async function findExistingView(entityID: string, user: UserInfo): Promise<MJUserViewEntityExtended | null> {
    const existing = await new RunView().RunView<MJUserViewEntityExtended>({
        ...
        ExtraFilter: `EntityID='${entityID}' AND Name='${VIEW_NAME.replace(/'/g, "''")}'`,
...
async function createView(entityID: string, md: Metadata, user: UserInfo): Promise<MJUserViewEntityExtended> {
    ...
    // `mj sync`. metadata/applications/.big-box-retail-application.json's second nav item
    // hardcodes this exact GUID as its RecordID; without setting it explicitly here,
    // BaseEntity would leave the uniqueidentifier PK null on NewRecord() and the server
    // would mint a random GUID at save time, breaking the Application's nav item on a
    // fresh environment/rebuild.
    view.ID = '66127631-8F91-4691-8F8E-C9B54437522E';
    view.UserID = user.ID;
    view.EntityID = entityID;
    view.Name = VIEW_NAME;
    view.Description = 'Default grid of all 389 Big Box Retail stores with multi-year assessed value and tax liability.';
    view.IsShared = true;
    view.IsDefault = true;
...
async function main(): Promise<void> {
    ...
    if (!entity) throw new Error(`Entity "${ENTITY_NAME}" not found — did CodeGen run (Task 3)?`);
